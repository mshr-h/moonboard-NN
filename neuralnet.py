...
def loaddata():
    ...
    # Load data
    loaddata = np.loadtxt("./climbs.txt")
    ...
    if np.shape(loaddata)[0] / rows != nproblems:
        print(
            'Mismatch with number of problems and number of grades!',
            file=sys.stderr)
        sys.exit()
    ...
    for i in range(nproblems):
        problemi = loaddata[i * rows:(i + 1) * rows][:].reshape(
            1, nholds)[0].tolist()
        if grades[i] >= low_grade and grades[
                i] <= high_grade:  # Only look at problems in some range
            data.append(problemi)

    # Remove the grades outside of the range of interest, but preserve the order!!!
    grades = [x for x in grades.tolist() if low_grade <= x <= high_grade]
